...
export function FixIncomePage({ apiBaseUrl }: FixedIncomePageProps): JSX.Element {
  ...
  const filterPresets = useMemo<FixedIncomeFilterPreset[]>(
    () => [
      {
        label: 'US Treasuries',
        description: 'Government yields quoted in USD from US venues.',
        filters: { country: 'United States', currency: 'USD' }
      },
      {
        label: 'Global bonds',
        description: 'Non-US sovereign or corporate listings.',
        filters: { country: '', type: 'bond' }
      }
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    const url = `${apiBaseUrl}/api/fixincome?${params.toString()}`;
    const startTime = performance.now();

    const loadFixedIncome = async (): Promise<void> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        if (!response.ok) {
          throw new ApiError('Failed to load fixed income instruments.', response.status);
        }

        const payload = (await response.json()) as FixedIncomeApiResponse;
        ...
        logError(fetchError as Error, { context: 'fixed-income-table' });
        setError(
          fetchError instanceof Error ? fetchError : new Error('Unable to load fixed income catalog right now.')
        );
        setAssets([]);
      } finally {
        setLoading(false);
  ...
  const handleToggleWatch = async ({ symbol, exchange, watched, asset }: ToggleWatchRequest): Promise<void> => {
    ...
    try {
      ...
      logError(toggleErrorCandidate, { context: 'fixed-income-watch-toggle', symbol, exchange: normalizedExchange });
      toggleError =
    ...
    <section className="page-shell" aria-label="Fixed income catalog">
      ...
      {error ? (
        <div role="alert" className="error-message">
          {error.message || 'Failed to load fixed income instruments.'}
          <div className="inline-actions">
      ...
      <GlobalLoadingShell visible={isInitialLoad} message="Loading fixed income instruments…" />
