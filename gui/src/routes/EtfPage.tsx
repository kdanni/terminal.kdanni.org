...
export function EtfPage({ apiBaseUrl }: EtfPageProps): JSX.Element {
  ...
  const filterPresets = useMemo<EtfFilterPreset[]>(
    () => [
      {
        label: 'US-listed ETFs',
        description: 'Focus on US-domiciled ETFs quoted in USD.',
        filters: { country: 'United States', currency: 'USD' }
      },
      {
        label: 'Major US exchanges',
        description: 'Highlight ETFs listed on NYSE, NASDAQ, or AMEX.',
        filters: { exchange: 'NYSE,NASDAQ,AMEX', country: 'United States' }
      }
    ],
    []
  );
  ...
  useEffect(() => {
    ...
    const url = `${apiBaseUrl}/api/etf?${params.toString()}`;
    ...
    const loadEtfs = async (): Promise<void> => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new ApiError('Failed to load ETFs from the catalog.', response.status);
        }

        const payload = (await response.json()) as EtfApiResponse;
        ...
        logError(fetchError as Error, { context: 'etf-table' });
        setError(fetchError instanceof Error ? fetchError : new Error('Unable to load ETF catalog.'));
        setAssets([]);
      } finally {
        setLoading(false);
      }
  ...
  }, [apiBaseUrl, countryFilter, currencyFilter, exchangeFilter, fetchWithAuth, nameFilter, page, reloadIndex, tickerFilter]);
  ...
  const applyPresetFilters = (preset: EtfFilterPreset): void => {
    const next = new URLSearchParams(searchParams);
  ...
  const handleToggleWatch = async ({ symbol, exchange, watched, asset }: ToggleWatchRequest): Promise<void> => {
    ...
    try {
      ...
      logError(toggleErrorCandidate, { context: 'etf-watch-toggle', symbol, exchange: normalizedExchange });
      toggleError =
  ...
  return (
    <section className="page-shell" aria-busy={loading}>
      <Breadcrumbs items={[{ label: 'Asset Catalog', path: '/catalog' }, { label: 'ETFs' }]} />
      <header className="page-header">
        <p className="page-kicker">Catalog</p>
        <h1>ETF data table</h1>
        <p className="app-description">
          Explore exchange-traded funds with filters for ticker, name, exchange, country, and currency. Use the watch toggle to track listings that matter.
        </p>
      ...
      <div className="filter-panel" aria-label="ETF filters">
      ...
      {error ? (
        <div role="alert" className="error-message">
          {error.message || 'Failed to load ETFs.'}
          <div className="inline-actions">
      ...
      <GlobalLoadingShell visible={isInitialLoad} message="Loading ETFs…" />
