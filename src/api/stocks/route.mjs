...
function buildFilters(query) {
  ...
  if (search) {
    ...
    conditions.push('(');
    conditions.push('  LOWER(stocks.symbol) LIKE ?');
    conditions.push('  OR LOWER(stocks.name) LIKE ?');
    conditions.push(')');
  ...
  if (name) {
    conditions.push('LOWER(stocks.name) LIKE ?');
...
router.get('/', async (req, res, next) => {
  ...
  const dataQuery = `
    ...
    FROM stocks_list stocks
    LEFT JOIN watch_list wl
      ON wl.symbol = stocks.symbol
     AND wl.exchange = COALESCE(stocks.exchange, '')
    ${clause}
